...
- **One line** per entry — two only if it genuinely needs a "why". Common-changelog
  ...
  of enumerating every affected tool/field by name (past 2-3 names, say "a
  keyless Web API tool" rather than listing all seven).
...
3. **Group, but also split**: commits belonging to the same logical change
   ...
   above gets blown past — it feels natural because it mirrors the commit
