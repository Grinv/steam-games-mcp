...
- Fix `find_friends_who_own`'s description crediting its FULL-library comparison to `get_friend_list` (a friends list, no game/playtime cap at all) instead of `get_owned_games` (the actual top-50-by-playtime cap it meant). ([fc003b5](https://github.com/Grinv/steam-games-mcp/commit/fc003b5))
...
- Fix `discover_games`'s and `get_recommended_games`'s `count` field descriptions not stating their own max (200 and 25 respectively), unlike every sibling numeric-range field in this codebase. ([fc003b5](https://github.com/Grinv/steam-games-mcp/commit/fc003b5))
