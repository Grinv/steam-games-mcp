...
- Fix a 403 from `get_game_news`, `get_current_players`, `get_followed_games`, `get_wishlist`, `get_items`, `discover_games` or `get_recommended_games` being blamed on invalid credentials whenever `STEAM_API_KEY` happens to be configured, even though none of these tools require a key. ([839649d](https://github.com/Grinv/steam-games-mcp/commit/839649d), [PLACEHOLDER](https://github.com/Grinv/steam-games-mcp/commit/PLACEHOLDER))
